...
void load_filenames(char *namelistfile, char *fnames)
{
  FILE *f;
  FILE *g;
...
void switchto_openbsd(char *slowfile, char **filenames, int nfiles, char *tgt)
{
  struct stat buff;
  char old[1000];
  char new[1000];
...
//  logprintf(attacker_log, "A: before rename");
  if(strcmp("/tmp/public", tgt) == 0){
	  strcat(new, "/tmp/good/");
	  strcat(new, fnames1[count1++]);
  }
  if(strcmp("/tmp/secret", tgt) == 0){
	  strcat(new, "/tmp/bad/");
	  strcat(new, fnames2[count2++]);
  }
  rename(old, new);
...
{ 
  ...
  if (conf->fork_switch)
    ...
    conf->switchfunc(slowfile, filenames, nfiles, tgt);
...
int main(int argc, char **argv)
{
  ...
  load_filenames(namelistfile, fnames);
  ...
  while(1) {
    // VICTIM: lstat
    do_one_step(victimpid, 2, slowfile, filenames, nfiles, goodfile, &config->steps[0]);
    // VICTIM: access
    do_one_step(victimpid, 3, slowfile, filenames, nfiles, badfile, &config->steps[1]);
    // VICTIM: open
    do_one_step(victimpid, 4, slowfile, filenames, nfiles, badfile, &config->steps[2]);
    // VICTIM: fsat
    if (config->steps[3].switchfunc)
      do_one_step(victimpid, 5, slowfile, filenames, nfiles, badfile, &config->steps[3]);
    // VICTIM: [close]
  }
